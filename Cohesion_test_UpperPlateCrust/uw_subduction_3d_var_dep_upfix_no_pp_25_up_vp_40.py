...
up_coh_dim = 40
up_coh_nd = np.round(up_coh_dim/pressure_scaling_MPa, 3)
# print(coh_nd)
...
outputPath = os.path.join(os.path.abspath("/scratch/n69/tg7098/"),'uw_subduction_3d_var_dep_upfix_no_pp_'+res+'_'+str(coh_dim)+'_up_vp_'+str(up_coh_dim)+'/')
if uw.mpi.rank ==0:
...
yielding_up         = 0.5 * tao_Y_up / (strainRate_2ndInvariant+1.0e-18)
...
if viscoplastic:
    ...
    upYieldvisc = fn.exception.SafeMaths(fn.misc.max(eta_min, fn.misc.min(SPCrustViscosity, yielding_up)))
...
steps_output            = 2   # output every 1 timesteps
...
while step < maxSteps:
    ...
    if step == maxSteps-1: # output intervals
        pol_con.repopulate()
        checkpoint()
        Vrms = math.sqrt( velSquared.evaluate()[0]/area.evaluate()[0] )
        if uw.mpi.rank==0:
